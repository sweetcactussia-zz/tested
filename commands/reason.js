...
exports.run = async (client, message, args) => {
  ...
  await modlog.fetchMessages({limit:100}).then((messages) => {
    ...
    modlog.fetchMessage(caseLog.id).then(logMsg => {
      ...
      embedSan(embed);
      embed.description = embed.description.replace(`Attente du modérateur. Utilisez ${settings.prefix}reason ${caseNumber} <raison>.`, newReason);
      logMsg.edit({embed});
    });
  });
};
...
exports.help = {
  name: 'reason',
  description: 'Met a jour une action dun modérateur',
